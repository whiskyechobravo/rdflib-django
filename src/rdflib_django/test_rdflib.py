...
class GraphTest(test.TestCase):
    ...
    def testSub(self):
        ...
        self.assertEqual((self.tarek, self.likes, self.pizza) in g3, True)
        self.assertEqual((self.tarek, self.likes, self.cheese) in g3, False)

        self.assertEqual((self.bob, self.likes, self.cheese) in g3, False)

        g1 -= g2

        self.assertEqual(len(g1), 1)
        self.assertEqual((self.tarek, self.likes, self.pizza) in g1, True)
        self.assertEqual((self.tarek, self.likes, self.cheese) in g1, False)

        self.assertEqual((self.bob, self.likes, self.cheese) in g1, False)
    ...
    def testGraphAdd(self):
        ...
        self.assertEqual((self.bob, self.likes, self.cheese) in g3, True)
        ...
        self.assertEqual((self.bob, self.likes, self.cheese) in g1, True)
    ...
    def testGraphIntersection(self):
        ...
        self.assertEqual(len(g3), 1)
        self.assertEqual((self.tarek, self.likes, self.pizza) in g3, False)
        self.assertEqual((self.tarek, self.likes, self.cheese) in g3, False)

        self.assertEqual((self.bob, self.likes, self.cheese) in g3, False)

        self.assertEqual((self.michel, self.likes, self.cheese) in g3, True)

        g1 *= g2

        self.assertEqual(len(g1), 1)

        self.assertEqual((self.tarek, self.likes, self.pizza) in g1, False)
        self.assertEqual((self.tarek, self.likes, self.cheese) in g1, False)

        self.assertEqual((self.bob, self.likes, self.cheese) in g1, False)

        self.assertEqual((self.michel, self.likes, self.cheese) in g1, True)
...
class ContextTest(test.TestCase):
    ...
    def testTriples(self):
        ...
        self.assertEqual(len(list(c1triples((Any, self.likes, self.pizza)))), 2)
        self.assertEqual(len(list(c1triples((Any, self.hates, self.pizza)))), 1)
        self.assertEqual(len(list(c1triples((Any, self.likes, self.cheese)))), 3)
        self.assertEqual(len(list(c1triples((Any, self.hates, self.cheese)))), 0)

        # unbound subjects without context, same results!
        self.assertEqual(len(list(triples((Any, self.likes, self.pizza)))), 2)
        self.assertEqual(len(list(triples((Any, self.hates, self.pizza)))), 1)
        self.assertEqual(len(list(triples((Any, self.likes, self.cheese)))), 3)
        self.assertEqual(len(list(triples((Any, self.hates, self.cheese)))), 0)

        # unbound objects with context
        self.assertEqual(len(list(c1triples((self.michel, self.likes, Any)))), 2)
        self.assertEqual(len(list(c1triples((self.tarek, self.likes, Any)))), 2)
        self.assertEqual(len(list(c1triples((self.bob, self.hates, Any)))), 2)
        self.assertEqual(len(list(c1triples((self.bob, self.likes, Any)))), 1)

        # unbound objects without context, same results!
        self.assertEqual(len(list(triples((self.michel, self.likes, Any)))), 2)
        self.assertEqual(len(list(triples((self.tarek, self.likes, Any)))), 2)
        self.assertEqual(len(list(triples((self.bob, self.hates, Any)))), 2)
        self.assertEqual(len(list(triples((self.bob, self.likes, Any)))), 1)

        # unbound predicates with context
        self.assertEqual(len(list(c1triples((self.michel, Any, self.cheese)))), 1)
        self.assertEqual(len(list(c1triples((self.tarek, Any, self.cheese)))), 1)
        self.assertEqual(len(list(c1triples((self.bob, Any, self.pizza)))), 1)
        self.assertEqual(len(list(c1triples((self.bob, Any, self.michel)))), 1)

        # unbound predicates without context, same results!
        self.assertEqual(len(list(triples((self.michel, Any, self.cheese)))), 1)
        self.assertEqual(len(list(triples((self.tarek, Any, self.cheese)))), 1)
        self.assertEqual(len(list(triples((self.bob, Any, self.pizza)))), 1)
        self.assertEqual(len(list(triples((self.bob, Any, self.michel)))), 1)

        # unbound subject, objects with context
        self.assertEqual(len(list(c1triples((Any, self.hates, Any)))), 2)
        self.assertEqual(len(list(c1triples((Any, self.likes, Any)))), 5)

        # unbound subject, objects without context, same results!
        self.assertEqual(len(list(triples((Any, self.hates, Any)))), 2)
        self.assertEqual(len(list(triples((Any, self.likes, Any)))), 5)

        # unbound predicates, objects with context
        self.assertEqual(len(list(c1triples((self.michel, Any, Any)))), 2)
        self.assertEqual(len(list(c1triples((self.bob, Any, Any)))), 3)
        self.assertEqual(len(list(c1triples((self.tarek, Any, Any)))), 2)

        # unbound predicates, objects without context, same results!
        self.assertEqual(len(list(triples((self.michel, Any, Any)))), 2)
        self.assertEqual(len(list(triples((self.bob, Any, Any)))), 3)
        self.assertEqual(len(list(triples((self.tarek, Any, Any)))), 2)

        # unbound subjects, predicates with context
        self.assertEqual(len(list(c1triples((Any, Any, self.pizza)))), 3)
        self.assertEqual(len(list(c1triples((Any, Any, self.cheese)))), 3)
        self.assertEqual(len(list(c1triples((Any, Any, self.michel)))), 1)

        # unbound subjects, predicates without context, same results!
        self.assertEqual(len(list(triples((Any, Any, self.pizza)))), 3)
        self.assertEqual(len(list(triples((Any, Any, self.cheese)))), 3)
        self.assertEqual(len(list(triples((Any, Any, self.michel)))), 1)
        ...
        for c in [graph, self.get_context(self.c1)]:
            # unbound subjects
            self.assertEqual(set(c.subjects(self.likes, self.pizza)), {self.michel, self.tarek})
            self.assertEqual(set(c.subjects(self.hates, self.pizza)), {self.bob})
            self.assertEqual(set(c.subjects(self.likes, self.cheese)), {self.tarek, self.bob, self.michel})
            self.assertEqual(set(c.subjects(self.hates, self.cheese)), set())

            # unbound objects
            self.assertEqual(set(c.objects(self.michel, self.likes)), {self.cheese, self.pizza})
            self.assertEqual(set(c.objects(self.tarek, self.likes)), {self.cheese, self.pizza})
            self.assertEqual(set(c.objects(self.bob, self.hates)), {self.michel, self.pizza})
            self.assertEqual(set(c.objects(self.bob, self.likes)), {self.cheese})

            # unbound predicates
            self.assertEqual(set(c.predicates(self.michel, self.cheese)), {self.likes})
            self.assertEqual(set(c.predicates(self.tarek, self.cheese)), {self.likes})
            self.assertEqual(set(c.predicates(self.bob, self.pizza)), {self.hates})
            self.assertEqual(set(c.predicates(self.bob, self.michel)), {self.hates})

            self.assertEqual(set(c.subject_objects(self.hates)), {(self.bob, self.pizza), (self.bob, self.michel)})
            self.assertEqual(set(c.subject_objects(self.likes)),
                              {(self.tarek, self.cheese), (self.michel, self.cheese), (self.michel, self.pizza), (self.bob, self.cheese),
                               (self.tarek, self.pizza)})

            self.assertEqual(set(c.predicate_objects(self.michel)), {(self.likes, self.cheese), (self.likes, self.pizza)})
            self.assertEqual(set(c.predicate_objects(self.bob)), {(self.likes, self.cheese), (self.hates, self.pizza), (self.hates, self.michel)})
            self.assertEqual(set(c.predicate_objects(self.tarek)), {(self.likes, self.cheese), (self.likes, self.pizza)})

            self.assertEqual(set(c.subject_predicates(self.pizza)), {(self.bob, self.hates), (self.tarek, self.likes), (self.michel, self.likes)})
            self.assertEqual(set(c.subject_predicates(self.cheese)), {(self.bob, self.likes), (self.tarek, self.likes), (self.michel, self.likes)})
            self.assertEqual(set(c.subject_predicates(self.michel)), {(self.bob, self.hates)})

            self.assertEqual(set(c), {(self.bob, self.hates, self.michel), (self.bob, self.likes, self.cheese), (self.tarek, self.likes, self.pizza),
